...
#include "alter_table_action_kind.h"

namespace mizugaki::ast::statement {

/**
 * @brief action in alter table statement.
 * @note `11.10 <alter table statement>` - `<alter table action>`
 * @see alter_table_statement
 */
class alter_table_action : public node {
public:
    /// @brief the node kind type.
    using node_kind_type = alter_table_action_kind;

    using node::node;
    [[nodiscard]] alter_table_action* clone() const& override = 0;
    [[nodiscard]] alter_table_action* clone() && override = 0;
     ...
     */
    friend bool operator==(alter_table_action const& a, alter_table_action const& b) noexcept;
...
protected:
     ...
     * @return false otherwise
     */
    [[nodiscard]] virtual bool equals(alter_table_action const& other) const noexcept = 0;
};
