...
TEST_F(analyze_query_expression_binary_test, except_all) {
    auto table_a = install_table("a");
    auto table_b = install_table("b");
    trelation::graph_type graph {};
    ...
    auto r = analyze_query_expression(
            ...
            ast::query::binary_expression {
                    ...
                    ast::query::binary_operator::except,
                    ast::query::set_quantifier::all,
                    ast::query::query {
                            {
                                    ast::query::select_asterisk {},
    ...
    // (scan - project)x2 - difference[all]
    auto&& binary = downcast<trelation::intermediate::difference>(r.output().owner());
    auto&& left_project = *find_prev<trelation::project>(binary.left());
    auto&& left_scan = *find_prev<trelation::scan>(left_project);
    auto&& right_project = *find_prev<trelation::project>(binary.right());
    auto&& right_scan = *find_prev<trelation::scan>(right_project);
    ...
    EXPECT_EQ(binary.quantifier(), trelation::set_quantifier::all);
    auto&& columns = binary.group_key_pairs();
    ...
    EXPECT_EQ(columns[0].left(), relation_columns[0].variable());
    EXPECT_EQ(columns[1].left(), relation_columns[1].variable());
    EXPECT_EQ(columns[2].left(), relation_columns[2].variable());
    EXPECT_EQ(columns[3].left(), relation_columns[3].variable());
}

TEST_F(analyze_query_expression_binary_test, except_distinct) {
    auto table_a = install_table("a");
    auto table_b = install_table("b");
    trelation::graph_type graph {};
    ...
    EXPECT_EQ(binary.quantifier(), trelation::set_quantifier::distinct);
    auto&& columns = binary.group_key_pairs();
    ASSERT_EQ(columns.size(), 4);
    {
        auto&& column = columns[0];
...
TEST_F(analyze_query_expression_binary_test, except_default) {
    auto table_a = install_table("a");
    auto table_b = install_table("b");
    trelation::graph_type graph {};
...
TEST_F(analyze_query_expression_binary_test, except_unsupported) {
    invalid(sql_analyzer_code::unsupported_feature, ast::query::binary_expression {
            ast::query::table_value_constructor {
                    ast::scalar::value_constructor {
                            literal(number("100")),
...
TEST_F(analyze_query_expression_binary_test, except_left_broken) {
    invalid(ast::query::binary_expression {
            ast::query::table_value_constructor {
                    ast::scalar::value_constructor {
                            vref(id("MISSING")),
...
TEST_F(analyze_query_expression_binary_test, except_right_broken) {
    invalid(ast::query::binary_expression {
            ast::query::table_value_constructor {
                    ast::scalar::value_constructor {
                            literal(number("100")),
...
TEST_F(analyze_query_expression_binary_test, except_inconsistent_columns) {
    invalid(sql_analyzer_code::inconsistent_columns, ast::query::binary_expression {
            ast::query::table_value_constructor {
                    ast::scalar::value_constructor {
                            literal(number("100")),
...
TEST_F(analyze_query_expression_binary_test, intersect_all) {
    auto table_a = install_table("a");
    auto table_b = install_table("b");
    trelation::graph_type graph {};
    ...
    auto r = analyze_query_expression(
            ...
            ast::query::binary_expression {
                    ...
                    ast::query::binary_operator::intersect,
                    ast::query::set_quantifier::all,
                    ast::query::query {
                            {
                                    ast::query::select_asterisk {},
    ...
    // (scan - project)x2 - difference[all]
    auto&& binary = downcast<trelation::intermediate::intersection>(r.output().owner());
    auto&& left_project = *find_prev<trelation::project>(binary.left());
    auto&& left_scan = *find_prev<trelation::scan>(left_project);
    auto&& right_project = *find_prev<trelation::project>(binary.right());
    auto&& right_scan = *find_prev<trelation::scan>(right_project);
...
TEST_F(analyze_query_expression_binary_test, intersect_distinct) {
    auto table_a = install_table("a");
    auto table_b = install_table("b");
    trelation::graph_type graph {};
