...
class analyze_type_test : public ::testing::Test {
public:
    ...
    analyzer_context::options_type& options() {
        return options_;
    }

    std::optional<analyzer_context::diagnostic_type> find_error(error_code code) {
        auto iter = std::find_if(
                context().diagnostics().begin(),
                context().diagnostics().end(),
                [=](auto&& found) { return found.code() == code; });
        if (iter == context().diagnostics().end()) {
            return std::nullopt;
        }
        return *iter;
    }
...
TEST_F(analyze_type_test, character_varying) {
    options().default_character_string_length() = 16;
    auto r = analyze_type(
            context(),
            ast::type::character_string {
    ...
    ASSERT_TRUE(r);
    EXPECT_EQ(*r, (ttype::character { ttype::varying, 16 }));
}
...
TEST_F(analyze_type_test, bit_varying) {
    options().default_bit_string_length() = 16;
    auto r = analyze_type(
            context(),
            ast::type::bit_string {
    ...
    ASSERT_TRUE(r);
    EXPECT_EQ(*r, (ttype::bit { ttype::varying, 16 }));
}
...
TEST_F(analyze_type_test, octet_varying) {
    options().default_octet_string_length() = 16;
    auto r = analyze_type(
            context(),
            ast::type::octet_string {
    ...
    ASSERT_TRUE(r);
    EXPECT_EQ(*r, (ttype::octet { ttype::varying, 16 }));
}
...
TEST_F(analyze_type_test, numeric) {
    options().default_decimal_precision() = 20;
    auto r = analyze_type(
            context(),
            ast::type::numeric {
...
TEST_F(analyze_type_test, decimal_flexible_both) {
    ...
    ASSERT_TRUE(r);
    EXPECT_EQ(*r, (ttype::decimal { {}, 0 })); // FIXME impl
}
