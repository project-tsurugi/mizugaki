...
TEST_F(scalar_dispatch_test, expression) {
    ...
    cb c;
    EXPECT_EQ(dispatch(c, as_lvref<expression>(variable_reference { id() })), 0);
    EXPECT_EQ(dispatch(c, as_lvref<expression>(unary_expression { unary_operator::plus, vref() })), 1);
    EXPECT_EQ(dispatch(c, as_lvref<expression>(comparison_predicate { vref(), comparison_operator::equals, vref() })), -1);
}
