...
public:
    ...
    [[nodiscard]] result_type operator()(ast::type::character_string const& type) {
        ...
        if (type.is_flexible_length()) {
            ...
            if (len.value().value() > options().max_character_string_length()) {
                context_.report(
                        sql_analyzer_code::type_length_is_too_large,
                        string_builder {}
                                << "too large character length: " << len.value().value()
                                << " (max length is " << options().max_character_string_length() << ")"
                                << string_builder::to_string,
                        len.value().region());
                return {};
            }
            ...
            if (type.is_varying()) {
                size = options().default_character_string_length();
            } else {
                size = 1;
            }
        }
    ...
    [[nodiscard]] result_type operator()(ast::type::bit_string const& type) {
        ...
        if (type.is_flexible_length()) {
            ...
            if (len.value().value() > options().max_bit_string_length()) {
                context_.report(
                        sql_analyzer_code::type_length_is_too_large,
                        string_builder {}
                                << "too large bit length: " << len.value().value()
                                << " (max length is " << options().max_bit_string_length() << ")"
                                << string_builder::to_string,
                        len.value().region());
                return {};
            }
            ...
            if (type.is_varying()) {
                size = options().default_bit_string_length();
            } else {
                size = 1;
            }
        }
    ...
    [[nodiscard]] result_type operator()(ast::type::octet_string const& type) {
        ...
        if (type.is_flexible_length()) {
            ...
            if (len.value().value() > options().max_octet_string_length()) {
                context_.report(
                        sql_analyzer_code::type_length_is_too_large,
                        string_builder {}
                                << "too large octet length: " << len.value().value()
                                << " (max length is " << options().max_octet_string_length() << ")"
                                << string_builder::to_string,
                        len.value().region());
                return {};
            }
            ...
            if (type.is_varying()) {
                size = options().default_octet_string_length();
            } else {
                size = 1;
            }
        }
    ...
    [[nodiscard]] result_type operator()(ast::type::decimal const& type) {
        ...
        return build(type, ttype::decimal {
            precision,
            scale.value_or(0), // FIXME support flexible precisions
        });
    }
    ...
    [[nodiscard]] result_type operator()(ast::type::datetime const& type) {
        using k = ast::type::kind;
        if (type.has_time_zone()) {
            // FIXME: timezone
            context_.report(
                    sql_analyzer_code::unsupported_feature,
                    string_builder {}
                            << "unsupported type feature (timezone): " << type
                            << string_builder::to_string,
                    type.region());
            return {};
        }
        switch (type.node_kind()) {
            case k::time:
                return build(type, ttype::time_of_day {});
            case k::timestamp:
                return build(type, ttype::time_point {});
            default:
                break;
        }
        fail();
    ...
    [[nodiscard]] result_type operator()(ast::type::interval const& type) {
        ...
        return build(type, ttype::datetime_interval {});
    }
