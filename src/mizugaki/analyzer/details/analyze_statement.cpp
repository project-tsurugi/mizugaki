...
public:
    ...
    [[nodiscard]] result_type operator()(ast::statement::insert_statement const& stmt) {
        ...
        for (auto&& key : index->keys()) {
            ...
            if (iter == destination_columns.end()) {
                // NOTE: key columns may not found in insertion columns for default values or implicit columns
                continue;
            }
    ...
    [[nodiscard]] result_type operator()(ast::statement::table_definition const& stmt) {
        ...
        for (auto&& element : stmt.elements()) {
            ...
            auto prototype = std::make_unique<::yugawara::storage::column>(
                    ...
                    ::yugawara::storage::column_value {}, // column value
            ...
            for (auto&& constraint_def : column.constraints()) {
                ...
                switch (constraint.node_kind()) {
                    ...
                    case kind::null:
                        ...
                        if (!saw_default) {
                            prototype->default_value() = ::yugawara::storage::column_value {
    ...
    [[nodiscard]] std::shared_ptr<::yugawara::storage::index> process_primary_key(
        ...
        auto relation = build_relation_info(*table);
        query_scope scope {};
        scope.add(relation);
    ...
    [[nodiscard]] result_type operator()(ast::statement::index_definition const& stmt) {
        ...
        auto relation = build_relation_info(*table_ptr);
        query_scope scope {};
        scope.add(relation);
