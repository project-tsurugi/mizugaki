...
public:
    ...
    [[nodiscard]] result_type operator()(ast::statement::update_statement const& stmt) {
        ...
        auto write_values = create_vector<trelation::write::column>(stmt.elements().size());
        for (auto&& element : stmt.elements()) {
            ...
            // check whether to try updating keys or values
            auto iter_key = std::find_if(
                    write_keys.begin(), write_keys.end(),
                    [&](trelation::write::key const& mapping) -> bool {
                        auto&& dest = ::yugawara::binding::extract<::yugawara::storage::column>(mapping.destination());
                        return dest == column->declaration();
                    });
            if (iter_key != write_keys.end()) {
                iter_key->source() = variable;
            } else {
                write_values.emplace_back(std::move(variable), factory_(*column->declaration()));
            }
        }
