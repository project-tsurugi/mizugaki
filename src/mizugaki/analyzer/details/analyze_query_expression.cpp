...
public:
    ...
    [[nodiscard]] result_type operator()(
        ...
        // FIXME: treat set functions in order by
        optional_ptr<trelation::intermediate::limit> order_by {};
        if (auto&& specs = expr.order_by(); !specs.empty()) {
            ...
            for (auto&& spec : specs) {
                ...
                // FIXME: select may organize a new scope (+check aggregation)
                auto result = analyze_scalar_expression(context_, *spec.key(), scope);
                if (!result) {
                    return {};
                }
                // FIXME: complex ORDER BY key
                auto variable = extract_variable(*result);
                if (!variable) {
                    context_.report(
                            sql_analyzer_code::unsupported_feature,
                            "ORDER BY key must be a plain variable",
                            spec.region());
                    return {};
                }
                ...
                sort_keys.emplace_back(*variable, *direction);
            }
...
[[nodiscard]] relation_info build_relation_info(::yugawara::storage::table const& table, bool escape_columns) {
