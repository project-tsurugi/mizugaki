...
public:
    ...
    [[nodiscard]] result_type operator()(
        ...
        // WHERE ...
        if (expr.where()) {
            auto where = analyze_scalar_expression(context_, *expr.where(), scope);
            if (!where) {
                return {};
            }
        ...
        relation_info info;
        {
            auto&& project = graph_.emplace<trelation::project>(
                    // reserve capacity except for `SELECT *`
                    create_vector<trelation::project::column>(
                            expr.elements().size() >= 2 ? expr.elements().size() : 0));
        ...
        if (expr.limit()) {
            ...
            auto&& limit = graph_.emplace<trelation::intermediate::limit>(
                    *nrows,
                    create_vector<tdescriptor::variable>(),
                    create_vector<trelation::intermediate::limit::sort_key>());
            limit.region() = context_.convert(expr.limit()->region());
    ...
    [[nodiscard]] result_type operator()(
        ...
        for (auto&& row_value : expr.elements()) {
            if (auto row_ctor = as_row_value_constructor(*row_value)) {
                ...
                context_.report(
                        sql_analyzer_code::unsupported_feature,
                        string_builder {}
                                << "table value constructor element: " << *row_value
                                << string_builder::to_string,
                        expr.region());
                return {};
            }
        ...
        for (std::size_t i = 0; i < ncols; ++i) {
            columns.emplace_back(factory_.stream_variable(
    ...
    [[nodiscard]] optional_ptr<output_port> operator()(
        ...
        context_.report(
                sql_analyzer_code::unsupported_feature,
                string_builder {}
                        << "unsupported table expression: " << expr
                        << string_builder::to_string,
                expr.region());
        return {};
    }
    ...
    [[nodiscard]] static std::string const& maybe_name(std::unique_ptr<ast::name::simple> const& n) noexcept {
        if (n) {
            return n->identifier();
        }
    ...
    [[nodiscard]] select_element_result operator()(
            ...
            relation_info& info) {
        auto value = analyze_scalar_expression(context_, *elem.value(), scope);
        if (!value) {
            return {};
        }
        auto column = factory_.stream_variable(maybe_name(elem.name()));
        target.columns().emplace_back(column, value.release());
        info.add({
                {},
                std::move(column),
        ...
        return { true, value.saw_aggregate() };
    }
    ...
    [[nodiscard]] select_element_result operator()(
        ...
        context_.report(
                sql_analyzer_code::unsupported_feature,
                string_builder {}
                        << "unsupported select element: " << elem
                        << string_builder::to_string,
                elem.region());
        return {};
    }
    ...
    [[nodiscard]] result_type operator()(
        ...
        context_.report(
                sql_analyzer_code::unsupported_feature,
                string_builder {}
                        << "unsupported grouping element: " << elem
                        << string_builder::to_string,
                elem.region());
        return {};
    }
...
private:
    ...
    [[nodiscard]] select_element_result dispatch(
        ...
        return ast::query::dispatch(*this, elem, std::move(scope), project, relation);
    }

    [[nodiscard]] result_type dispatch(
            ast::query::grouping_element const& elem,
            optional_ptr<query_scope const> scope) {
        return ast::query::dispatch(*this, elem, std::move(scope));
    }
    ...
    [[nodiscard]] relation_info build_relation_info(::yugawara::storage::table const& table) {
        ...
        for (auto&& column : table.columns()) {
            result.add(column_info {
                    ...
                    factory_.stream_variable(
                            debug_string_builder {} << column << debug_string_builder::to_string
                    ),
    ...
    [[nodiscard]] bool rebuild_relation_info(relation_info& info, ast::table::correlation_clause const& correlation) {
        ...
        info.identifier() = correlation.correlation_name()->identifier();
        if (!correlation.column_names().empty()) {
            ...
            for (std::size_t index = 0, n = cor.size(); index < n; ++index) {
                rel[index].identifier() = cor[index]->identifier();
            }
