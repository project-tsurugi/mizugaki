...
public:
    ...
    std::unique_ptr<tscalar::expression> operator()(ast::literal::string const& value) {
        ...
        switch (value.node_kind()) {
            ...
            case kind::bit_string: return process_bit_string(value);
            case kind::hex_string: return process_hex_string(value);
            default: fail();
        }
    }
    ...
    std::unique_ptr<tscalar::expression> operator()(ast::literal::datetime const& value) {
        using ast::literal::kind;
        switch (value.node_kind()) {
            case kind::date: return process_date(value);
            case kind::time: return process_time(value);
            case kind::timestamp: return process_timestamp(value);
            default: fail();
        }
    }
...
private:
    ...
    [[nodiscard]] std::unique_ptr<tscalar::expression> process_approximate_numeric(ast::literal::numeric const& value) {
        (void) value;
        (void) context_;
        ::takatori::util::fail(); // FIXME: impl - stof?
    }
    ...
    [[nodiscard]] std::unique_ptr<tscalar::expression> process_character_string(ast::literal::string const& value) {
        ...
        if (nchars > context_.options()->max_character_string_length()) {
            context_.report(sql_analyzer_code::unsupported_string_value,
    ...
    [[nodiscard]] std::unique_ptr<tscalar::expression> process_bit_string(ast::literal::string const& value) {
        (void) value;
        (void) context_;
        ::takatori::util::fail(); // FIXME: impl
    }

    [[nodiscard]] std::unique_ptr<tscalar::expression> process_hex_string(ast::literal::string const& value) {
        (void) value;
        (void) context_;
        ::takatori::util::fail(); // FIXME: impl
    }

    [[nodiscard]] std::unique_ptr<tscalar::expression> process_date(ast::literal::datetime const& value) {
        (void) value;
        (void) context_;
        ::takatori::util::fail(); // FIXME: impl
    }

    [[nodiscard]] std::unique_ptr<tscalar::expression> process_time(ast::literal::datetime const& value) {
        (void) value;
        (void) context_;
        ::takatori::util::fail(); // FIXME: impl
    }

    [[nodiscard]] std::unique_ptr<tscalar::expression> process_timestamp(ast::literal::datetime const& value) {
        (void) value;
        (void) context_;
        ::takatori::util::fail(); // FIXME: impl
    }
